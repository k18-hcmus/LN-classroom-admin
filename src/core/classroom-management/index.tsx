...
import {
    Card, Container, IconButton, Stack, Table, TableBody,
...
const TABLE_HEAD = [
    ...
    { id: 'owner.username', label: 'Owner', alignRight: false },
    { id: 'schoolYear', label: 'School Year', alignRight: false },
...
function applySortFilter(array: any[], comparator: any, query: string) {
    ...
    if (query) {
        return filter(
            array,
            (_classroom) => doSearch([_classroom.name, _classroom.owner.username], query)
        );
    }
...
export default function ClassroomManagement() {
    ...
    useEffect(() => {
        dispatch(selectRoute(RouteName.CLASSROOM))
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - classrooms.length) : 0;

    const filteredClassrooms = applySortFilter(classrooms, getComparator(order, orderBy), filterName);

    const isUserNotFound = filteredClassrooms.length === 0;
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell align="right">
                                                    <IconButton component={RouterLink} to={`/classrooms/${_id}`} target="_blank" rel="noopener noreferrer">
                                                        <RemoveRedEyeIcon />
                                                    </IconButton >
                                                </TableCell>
                                            </TableRow>
